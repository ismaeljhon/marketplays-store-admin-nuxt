<template>
  <v-card>
    <v-card-title>
      <v-row dense>
        <v-col cols="5">
          <h3 class="mr-2"><v-icon left>mdi-domain</v-icon> Categories</h3>
        </v-col>
        <v-col cols="7" class="text-right">
          ...
          >
            <v-icon left>mdi-download</v-icon>Export Categories
          </download-csv>
          <NuxtLink to="/categories/add">
            <v-btn small color="primary">
              <v-icon left>mdi-plus</v-icon>Add Categories
            </v-btn>
          </NuxtLink>
        </v-col>
        <v-col cols="12">
    ...
    <v-card-text>
      ...
      >
        ...
        <template slot="item.teamLead" slot-scope="row">
          {{
            row.item.teamLead
              ? row.item.teamLead.firstName +
                ' ' +
                row.item.teamLead.middleName +
                ' ' +
                row.item.teamLead.lastName
              : '-'
          }}
        </template>
        <template slot="item.action" slot-scope="row">
          <v-tooltip top>
            <template v-slot:activator="{ on, attrs }">
              <NuxtLink :to="'/categories/' + row.item._id">
  ...
  methods: {
    deleteItems(items) {
      ...
      swal({
        ...
        if (willDelete) {
          const result = await this.deleteMutation('Category', items._id)
